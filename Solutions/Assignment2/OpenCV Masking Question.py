        ...
        for col in range(1, array.shape[1] - 1):
            if array[row, col] == 0:
                if array[row - 1, col] == 1:
                    dilated_array[row, col] = 1
                elif array[row + 1, col] == 1:
                    dilated_array[row, col] = 1
                elif array[row, col - 1] == 1:
                    dilated_array[row, col] = 1
                elif array[row, col + 1] == 1:
                    dilated_array[row, col] = 1
...
boolean_mask = (closed_after_opened_first[:, :] == 255)
processed_mask = np.array(first_frame)
processed_mask[np.invert(boolean_mask)] = 0
...
centroid_image[np.invert(boolean_mask)] = 0
